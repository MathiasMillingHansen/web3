...
class Game {
	...
	private _randomizer: (() => number) | undefined;
	private _cardsPerPlayer: number;
	private _dealer: number;
	private _rounds: Round[] = [];

	constructor({ players, targetScore, randomizer, cardsPerPlayer }: { players: string[], targetScore: number, randomizer?: () => number, cardsPerPlayer?: number }) {
		if (!players || players.length < 2) throw new Error('At least 2 players required');
		if (!targetScore || targetScore <= 0) throw new Error('Target score must be > 0');
	...
	player(i: number) { if (i < 0 || i >= this._players.length) throw new Error('Player index out of bounds'); return this._players[i]; }
	get targetScore() { return this._targetScore; }
	score(i: number) { if (i < 0 || i >= this._scores.length) throw new Error('Player index out of bounds'); return this._scores[i]; }
	winner() { return this._winner; }
	currentRound() { return this._round; }

	   private startRound(memento?: any) {
		   if (memento) {
			   this._round = new Round();
			   this._round.players = [...memento.players];
			   this._round.dealer = memento.dealer;
			   this._round.deck = new Deck();
			   this._round.deck.cards = memento.drawPile ? [...memento.drawPile] : [];
			   this._round.hands = (memento.hands || []).map((hand: Card[]) => new Hand([...hand]));
			...
			this._round.hands = this._players.map(() => new Hand());
